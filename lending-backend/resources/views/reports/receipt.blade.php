...
<body>
    ...
    <div class="details">
        <p><span class="label">Date:</span> <span>{{ $payment->payment_date->format('M d, Y') }}</span></p>
        <p><span class="label">Borrower:</span> <span>{{ $payment->loan->borrower->full_name }}</span></p>
        <p><span class="label">Loan Amount:</span> <span>₱{{ number_format($payment->loan->loan_amount, 2) }}</span></p>
    </div>
    ...
    <div class="details">
        <p><span class="label">Remaining Balance:</span> <span>₱{{ number_format($payment->balance_after, 2) }}</span></p>
        @if($payment->remarks)
        <p><span class="label">Remarks:</span> <span>{{ $payment->remarks }}</span></p>
        @endif
    </div>
